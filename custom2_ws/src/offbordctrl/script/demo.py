...
def setPoint_cb(msg):
    ...
    x=msg.data[0]
    y=msg.data[1]
    x = min(max(0.1,x),1.75)
    y = min(max(0,y),9)

    if abs(x-cur_x) > 0.15 or abs(y-cur_y) > 0.15:
        update_pos = True
        # print(x,y)
...
def sendTargetPos():
    ...
    while not rospy.is_shutdown():
        ...
        if stopThread:
            try:
                # x_arr = np.linspace(cur_x,0.94,100)
                # y_arr = np.linspace(cur_y,4.71,100)
...
if __name__ == '__main__':
    ...
    # rospy.Subscriber("/drone2SetPoint",Float32MultiArray,setPoint_cb)
