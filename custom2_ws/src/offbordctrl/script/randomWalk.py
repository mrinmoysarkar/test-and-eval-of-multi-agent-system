...
def sendTargetPos():
    ...
    setpoint.pose.position.x = cur_x
    setpoint.pose.position.y = cur_y
    ...
    while not rospy.is_shutdown():
        ...
        if stopThread:
            try:
                x_arr = np.linspace(cur_x,x_const,100)
                y_arr = np.linspace(cur_y,y_const,100)
...
if __name__ == '__main__':
    ...
    rospy.init_node('drone_randomwalk_custom2', anonymous=True)
    ...
    rospy.Subscriber("/vicon/jackal1/jackal1", TransformStamped, setPoint_cb)
    rospy.Subscriber("/custom2/mavros/battery", BatteryState, batterystate_cb)
    ...
    if cur_x != 0 and cur_y != 0:
        setpoint = PoseStamped()
        setpoint.pose.position.x = 0.94
        setpoint.pose.position.y = 1.65
        setpoint.pose.position.z = z
        ...
        if set_mode('OFFBOARD', 5) and set_arm(True, 5):
            ...
            while not rospy.is_shutdown():
                rate.sleep()
                cmd = raw_input("command: \"l\" for land and then \"exit\" for exit the main loop: ")
                # print(cmd)
                if cmd=='l':
                    stopThread = True
                elif cmd=='exit':
